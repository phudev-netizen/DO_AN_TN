...
class HoaDonBanVỉewModel: ViewModel() {
    ...
    // Thêm hóa đơn và lấy về mã hóa đơn mới tạo (sử dụng suspend fun để gọi từ coroutine)
    suspend fun addHoaDonAndGetId(hoaDonBan: HoaDonBan): Int {
        return try {
            val response = withContext(Dispatchers.IO) {
                QuanLyBanLaptopRetrofitClient.hoaDonBanAPIService.addHoaDonBanAndGetId(hoaDonBan)
            }
            if (response.success && response.MaHoaDonBan != null) {
                response.MaHoaDonBan // Trả về mã hóa đơn mới tạo
            } else {
                0 // Có thể trả về -1 để báo lỗi
            }
        } catch (e: Exception) {
            Log.e("HoaDonBanViewModel", "Lỗi tạo hóa đơn: ${e.message}")
            0
        }
    }
